...
`AGENTS.md` 的项目经验区只记录会改变未来默认行为的具体规则。推荐写成“Y 场景永远用 X”这类可执行约束，避免“注意质量”“小心上下文”这类抽象提醒。

- 已有规则能覆盖时，优先修订原规则，不追加近义重复项。
- 只对本次任务有用的过程噪音进入 session summary 或 archive，不进入长期 Agent 规则。
- 过时、被验证推翻或长期未使用的经验规则必须有可审计降权、归档或删除路径。
- 规则变更后按影响范围运行 docs sync、routing 或 ADK 相关验证，不把规则文本更新当作自动完成。
...
- 兼容 fallback 状态以 `docs/reference/fallback-sunset-matrix.md` 为准，不做无证据下线。
...
项目规则文件的职责是让 Agent 少猜，不是保存百科。适合写稳定技术栈、目录入口、测试命令、禁改区域、完成标准和常见陷阱；不适合写完整教程、工具榜单、一次性偏好、旧日志或未验证外部项目。

项目级 `AGENTS.md` 或同类规则文件推荐保持“地图页”形态，至少覆盖：

- `Project Map`：核心目录、入口、测试目录、生成物和禁读/禁改区域。
- `Read First`：进入关键目录、复杂流程或子系统前必须读取的本地规则和 runbook。
- `Commands`：安装、格式化、lint、typecheck、测试、构建的唯一推荐命令。
- `Rules`：稳定架构约束、依赖限制、兼容性要求和禁止绕过的审批/CI 流程。
- `Completion Rules`：不同变更类型对应的最小验证链路。
- `Escalate Before Changing`：公共 API、schema/migration、部署、安全、生产配置、新依赖和不可逆操作。

根级规则应短、稳、可执行；超过约 150 行时优先拆到目录级 `AGENTS.md`、`PROJECT_MAP.md` 或 runbook，不把完整部署手册、一次性任务、工具榜单和旧日志塞入根文档。
