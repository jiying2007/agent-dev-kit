...
- Codex/Hermes/Claude 类 plugin 或 Skill 包如果包含 hooks、MCP server、app integration、marketplace catalog 或 lifecycle scripts，必须逐项拆开审查；安装包可信不代表每个组件都可启用。
...
- 外部 AGENTS/CLAUDE/GEMINI 配置分享中的 MCP server、provider relay、API key、hook、命令行安装片段和平台协作命令默认属于运行态连接器候选；只能登记为 report-only，不能复制到 `manifest.yaml`、profile 或 `~/.codex`。
...
- 会写入用户目录、重建 `~/.codex` / `~/.claude` / `~/.agents`、创建符号链接投影、安装 marketplace 包、配置消息机器人、开启 cron/hook/web server 或远程命令执行的教程，默认 `reject`，除非另有完整供应链和运行态权限审查。
