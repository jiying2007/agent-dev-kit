...
Use official OpenAI Developers guidance as a current reference source while keeping `agent-dev-kit` deterministic, reviewable and locally validated.
...
| P0 | Codex task framing and reusable guidance | task card with Goal/Context/Constraints/Done-when, AGENTS/skill promotion rationale |
| P0 | Codex runtime policy, `requirements.toml`, sandbox defaults | runtime policy manifest, forbidden default list, runtime-boundary check |
| P0 | Codex command rules and Docs MCP setup | rules contract manifest, inline rule examples, cross-tool Docs MCP manifest |
...
| P1 | Hooks, slash commands and Codex-as-MCP contracts | hook audit manifest, slash command audit manifest, Codex MCP runner contract |
...
| P1 | Codex automations and worktrees | report-only automation records, first-run review, worktree handoff and cleanup gates |
| P1 | Agent improvement loop | traces, human/model feedback, eval candidate, validation gate, ranked recommendation and Codex handoff |
...
| P2 | Codex plugin and marketplace packaging | plugin marketplace contract, source path containment, install/auth policy review |
...
- Agent improvement loops include traces, feedback, eval candidate, validation result, ranked recommendations and a Codex handoff before changing prompts, skills or workflows.
...
5. Codex MCP: record `threadId`, `cwd`, sandbox, approval policy, profile and approval prompt correlation.
...
9. Docs MCP: use OpenAI Docs MCP first for OpenAI product/API questions across Codex, VS Code, Cursor and Claude Code; fallback only to official OpenAI domains.
10. Plugin marketplace: verify `.codex-plugin/plugin.json`, skill path, source path containment, install policy and auth policy.
...
17. Improvement loop: require trace-feedback-eval-validation-Codex handoff linkage and human approval before apply.
