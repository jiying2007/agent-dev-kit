...
下表记录 `global-dev-kit` 内部能力与实现映射，不依赖外部仓库上下文。
...
| 单问题与边界核验 | 单次变更聚焦单问题，并显式声明 Core/Optional 归属 | `proposal.md` 模板、`gdk-commit-pr-quality-gate` |
| 评审闭环 | blocker/major/minor 分级并归档前强校验 | `scripts/workflow.sh review`、`review-report.md` |
| 负结果留痕 | 记录被证伪假设与不采用方案 | `negative-results.md`、`gdk-systematic-debugging` |
