...
软件 M5 campaign 使用 `manifests/software_m5_eval_contract_rc7.json`。正式契约包含 60 个任务、Codex/Claude、baseline/ADK、3 trials、显式模型、最多一次错误重试和 `$150` 硬预算；逐任务结果原子落盘，恢复时拒绝 manifest/contract/task/plan/runtime 版本漂移。RC7 保留 RC6 的意图/工作项确定性回归并纳入 Token 与上下文治理资产，但双 runtime campaign 仍需单独执行，不能由本地 fixture 替代。

```bash
bash scripts/devkit.sh eval campaign plan --contract manifests/software_m5_eval_contract_rc7.json --summary-json
bash scripts/devkit.sh eval campaign run --contract manifests/software_m5_eval_contract_rc7.json --state-dir /tmp/adk-m5-campaign --execute --approve-budget-usd 150 --summary-json
bash scripts/devkit.sh eval campaign run --contract manifests/software_m5_eval_contract_rc7.json --state-dir /tmp/adk-m5-campaign --execute --approve-budget-usd 150 --resume --summary-json
bash scripts/devkit.sh eval certify --contract manifests/software_m5_eval_contract_rc7.json --state-dir /tmp/adk-m5-campaign --output /tmp/adk-m5-certification.json
...
bash scripts/devkit.sh release build --version 3.1.0-rc.7 --out dist --summary-json
bash scripts/devkit.sh release runtime-build --version 3.1.0-rc.7 --profile team-core --out dist --summary-json
bash scripts/devkit.sh release rehearse --previous-artifact /tmp/agent-dev-kit-3.1.0-rc.6.tar.gz --candidate-artifact dist/agent-dev-kit-3.1.0-rc.7.tar.gz --output /tmp/adk-release-rehearsal.json
bash scripts/devkit.sh release publish --version 3.1.0-rc.7 --backend github --artifact dist/agent-dev-kit-3.1.0-rc.7.tar.gz --dry-run
```
