...
本目录记录 adk-first 能力在真实或准真实任务中的运行证据，用于决定 fallback 是否可以从默认依赖降级到显式 fallback，最终再进入 sunset。
...
- 任务输入和背景。
- primary skill、supporting skill、fallback 裁决。
- 是否使用 fallback，若使用必须说明原因。
- 执行摘要、验证命令、退出码和证据路径。
