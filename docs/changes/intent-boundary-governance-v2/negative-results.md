...
| 2026-07-19 | T7 release | RC4/RC5 `release rehearse`（首次） | 当前 loader 用 RC5 target contract 规则 strict-validate RC4 source，旧 contract 缺 `skill_invocation` 导致 `KeyError`，rehearsal exit 1 | 判为 blocker；active loader 改为明确 `target_contract_incompatible`，release-only boundary 从校验过的 RC4 bundle 执行 hard-cut rollback/reinstall，不恢复 active compatibility reader |
