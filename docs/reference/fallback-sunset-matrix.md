...
- 需要归档评分明细时使用 `scripts/check-fallback-sunset.sh --score-tsv <path>`；需要低 token 摘要时使用 `--summary-json`。
- Pilot 证据成熟度单独用 `scripts/pilot-readiness.sh` 检查；需要低 token 摘要时使用 `scripts/pilot-readiness.sh --summary-json`。
- Pilot index 与 evidence 文件必须同步：`status:` 行必须与 `docs/pilots/index.tsv` 一致，planned pilot 必须有待补证据，ready pilot 必须有验证证据。
