...
| 验证/调试 | host unit、ctest、QEMU/SIL、HIL、静态分析、fault injection、GDB/OpenOCD/JTAG、trace | Evidence Index、日志、波形、测试报告 |
...
- 驱动 bring-up 默认使用 `adk-driver-bringup-checklist`，必要时叠加 `adk-register-map-design` 与 `adk-toolchain-debug-openocd-gdb`。
