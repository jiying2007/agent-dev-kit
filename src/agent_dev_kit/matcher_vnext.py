...
def resolve_skill_content(manifest: Manifest, skill_id: str) -> Dict[str, str]:
    ...
    if not isinstance(classes, dict) or not isinstance(ceilings, dict):
...
def _apply_contract(result: Mapping[str, Any], metadata: Mapping[str, str]) -> Dict[str, Any]:
    ...
    # explicit-approval result into a stronger permission.
    if metadata["effect_ceiling"] in _READONLY_CEILINGS:
        constrained["mutation_permission"] = "deny"
    return constrained


def _primary_in_group(manifest: Manifest, selection_group: str) -> Optional[tuple[str, str, Mapping[str, str]]]:
    candidates = []
...
def _next_primary_trigger(
    ...
    # family. Promote the same-group unique primary before scanning unrelated
    # primary triggers; ambiguity fails closed in _primary_in_group().
    if preferred_group:
...
def match_text(
    ...
    if result.get("source") in ("skill_trigger", "optional_skill_trigger"):
        ...
        abstained["reason"] = "no-primary-eligible-skill"
        return abstained
    return _apply_contract(result, metadata)
