...
4. 运行目标检查：若目标是 `~/.codex`，必须补运行目录健康验证证据。
...
## 与 gdk-commit-pr-quality-gate 的区别
- gdk-verification-before-completion: 完成前自检（验证命令、证据、边界）
- gdk-commit-pr-quality-gate: 提交/PR 质量门禁（格式、规范、评审）

## Quality Gate
- 输出必须包含验证命令、关键结果、风险项和处理状态。
- 若存在未闭环 blocker，结论必须为 `needs-fix`。
- 完成声明需与实际证据逐项可追溯。
- 若声明目标可在 `~/.codex` 放行，必须附运行目录健康验证结果。
