...
`manifest.json` 当前 source version 为 `5.1.0`。已发布组件请以 GitHub Releases 和 release contract 为准；`main` 可以在某个不可变 Release 之后继续前进，因此 **current main != latest immutable release** 是允许且预期的状态。
