...
# 边界条件测试 - unified devkit match command
...
test_empty_string() {
    local output
    output=$(match_cli --text "" 2>&1) || true
...
test_very_long_input() {
    ...
    local output
    output=$(match_cli --text "$long_text" 2>&1) || true
...
test_special_chars_double_quotes() {
    local output
    output=$(match_cli --text '输入包含"双引号"的文本' 2>&1) || true
    [[ "$output" == *"match="* ]]
}
...
test_special_chars_single_quotes() {
    local output
    output=$(match_cli --text "输入包含'单引号'的文本" 2>&1) || true
    [[ "$output" == *"match="* ]]
}
...
test_special_chars_backslash() {
    local output
    output=$(match_cli --text '路径C:\windows\system32' 2>&1) || true
    [[ "$output" == *"match="* ]]
}
...
test_special_chars_pipe() {
    local output
    output=$(match_cli --text 'echo "hello" | grep hello' 2>&1) || true
    [[ "$output" == *"match="* ]]
}
...
test_unicode_chinese() {
    local output
    output=$(match_cli --text '需求不清楚，需要澄清一下这个功能' 2>&1) || true
    [[ "$output" == *"match=true"* ]]
}
...
test_emoji_input() {
    local output
    output=$(match_cli --text '🚗 驱动开发需要帮忙 🚀' 2>&1) || true
...
test_unicode_mixed() {
    local output
    output=$(match_cli --text 'Write 单元测试 for module' 2>&1) || true
    [[ "$output" == *"match=true"* ]]
}
...
test_routing_02_task_breakdown() {
    local output
    output=$(match_cli --text "拆解大任务" 2>&1) || true
...
test_routing_03_interface_design() {
    local output
    output=$(match_cli --text "设计接口" 2>&1) || true
...
test_routing_05_static_analysis() {
    local output
    output=$(match_cli --text "静态分析" 2>&1) || true
...
test_routing_06_debugging() {
    local output
    output=$(match_cli --text "调试" 2>&1) || true
...
test_routing_07_ready_to_complete() {
    local output
    output=$(match_cli --text "准备完成" 2>&1) || true
...
test_routing_09_adr() {
    local output
    output=$(match_cli --text "写ADR" 2>&1) || true
...
test_routing_10_register_map() {
    local output
    output=$(match_cli --text "设计寄存器映射" 2>&1) || true
...
test_routing_11_driver_bringup() {
    local output
    output=$(match_cli --text "驱动开发" 2>&1) || true
...
test_routing_12b_boot_chain() {
    local output
    output=$(match_cli --text "启动链 Bootloader rootfs 最小启动" 2>&1) || true
    [[ "$output" == *"match=true"* && "$output" == *"skill=adk-bsp-porting-playbook"* ]]
}
...
test_routing_13_rtos_task() {
    local output
    output=$(match_cli --text "RTOS任务设计" 2>&1) || true
...
test_routing_14_interrupt_dma() {
    local output
    output=$(match_cli --text "中断处理" 2>&1) || true
...
test_routing_15_protocol_stack() {
    local output
    output=$(match_cli --text "协议栈集成" 2>&1) || true
...
test_routing_16_api_stability() {
    local output
    output=$(match_cli --text "API稳定性" 2>&1) || true
...
test_routing_17_cmake_cross() {
    local output
    output=$(match_cli --text "CMake交叉编译" 2>&1) || true
...
test_routing_18_embedded_debug_transport() {
    local output
    output=$(match_cli --text "ADB SSH 串口 GDB remote 调试通道" 2>&1) || true
...
test_routing_19_hil_sil() {
    local output
    output=$(match_cli --text "HIL/SIL集成测试" 2>&1) || true
...
test_routing_20_fault_injection() {
    local output
    output=$(match_cli --text "故障注入测试" 2>&1) || true
...
test_routing_22_release() {
    local output
    output=$(match_cli --text "版本发布" 2>&1) || true
...
test_routing_23_production_field() {
    local output
    output=$(match_cli --text "量产产测烧录诊断 OTA升级 回滚 现场维护" 2>&1) || true
    [[ "$output" == *"match=true"* && "$output" == *"skill=adk-production-field-readiness"* ]]
}

test_routing_23b_production_field_sim_device() {
    local output
    output=$(match_cli --text "production-field simulate-device 设备状态机" 2>&1) || true
    [[ "$output" == *"match=true"* && "$output" == *"skill=adk-production-field-readiness"* ]]
}
...
test_multi_slash_intent_zh_first() {
    ...
    local output
    output=$(match_cli --text "需求模糊" 2>&1) || true
    [[ "$output" == *"match=true"* && "$output" == *"skill=adk-requirements-triage"* ]]
}
...
test_multi_slash_intent_zh_last() {
    ...
    local output
    output=$(match_cli --text "需求不明确" 2>&1) || true
    [[ "$output" == *"match=true"* && "$output" == *"skill=adk-requirements-triage"* ]]
}
...
test_multi_slash_intent_zh_partial() {
    ...
    local output
    output=$(match_cli --text "DMA处理" 2>&1) || true
    [[ "$output" == *"match=true"* && "$output" == *"skill=adk-interrupt-dma-patterns"* ]]
}

test_routing_24_skill_governance() {
    local output
    output=$(match_cli --text "skill、workflow 分类和排序需要治理" 2>&1) || true
