...
for fixture in "$ROOT_DIR"/fixtures/runtime-capabilities/pass/*.json; do
  ...
  "$ROOT_DIR/scripts/check-runtime-capabilities.sh" --summary-json --fixture "$fixture" | rg -q '"status":"pass"'
done

for fixture in "$ROOT_DIR"/fixtures/runtime-capabilities/fail/*.json; do
  if "$ROOT_DIR/scripts/check-runtime-capabilities.sh" --fixture "$fixture" >/dev/null 2>&1; then
    echo "[FAIL] negative fixture passed unexpectedly: $fixture" >&2
    exit 1
  fi
  summary="$("$ROOT_DIR/scripts/check-runtime-capabilities.sh" --summary-json --fixture "$fixture" 2>/dev/null || true)"
  printf '%s\n' "$summary" | rg -q '"status":"fail"'
done

echo "[PASS] runtime capability gates test"
